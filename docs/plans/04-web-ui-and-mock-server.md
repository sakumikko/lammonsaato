...
- **Show:** Cold weather toggle (always visible), single block duration dropdown [5, 10, 15], info text "1 block per hour, 10 blocks total"
- **Hide:** Min block dropdown, Max block dropdown, Total hours dropdown
- **Keep visible:** Cost constraint field (secondary)
...
In `/api/calculate` handler: if `cold_weather_mode`, call `find_cold_weather_schedule()` from schedule_optimizer.py instead of the normal path.

Fix pre-existing bug: `/api/block-enabled` hardcoded range `1-4` should be `1-10`.

### scripts/mock_server/entity_signatures.json

Add 4 new entity signatures matching the YAML definitions.
